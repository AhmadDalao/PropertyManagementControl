...
class AssetDetailOverviewPresenter
{
    ...
    public function present(AssetDetailData $data): array
    {
        ...
        $lease = $operations->directActiveLease;
        ...
        $stats = [
            ...
            ['label' => trans('app.assets.occupancy_rate'), 'value' => $this->rate($operations->occupancyRate()), 'tone' => $operations->occupancyRate() >= 80 ? 'teal' : 'primary'],
            ['label' => trans('app.assets.rentable_spaces'), 'value' => $operations->rentableCount],
        ];

        if ($leasesEnabled) {
            $stats[] = ['label' => trans('app.assets.active_leases'), 'value' => $operations->activeLeaseCount, 'tone' => $operations->activeLeaseCount > 0 ? 'teal' : 'muted'];
        }
