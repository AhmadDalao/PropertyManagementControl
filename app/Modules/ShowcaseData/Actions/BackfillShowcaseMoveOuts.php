...
final readonly class BackfillShowcaseMoveOuts
{
    public function __construct(
        private AssetHierarchy $hierarchy,
        private ShowcaseMoveOutBuilder $moveOuts,
    ) {}
        ...
        ShowcaseDataset::query()
            ...
            ->each(function (ShowcaseDataset $dataset) use (&$generated): void {
                $buildings = Asset::query()
                    ->whereNull('parent_id')
                    ->whereHas('portfolio', fn ($portfolios) => $portfolios
                        ->where('showcase_dataset_id', $dataset->id))
                    ->orderBy('id')
                    ->get();

                foreach ($buildings as $buildingIndex => $building) {
                    $assetIds = $this->hierarchy->descendantIdsIncluding($building);
                    $leases = array_values(
                        Lease::query()
