...
**Application build revision verified in production:** tracked by `.deployed-revision` and checked against GitHub `main`
...
The application code is healthy. The release reran the complete PHP suite: **671 tests and 42,128 assertions passed**. The release baseline also includes **71 passing Playwright/axe scenarios**, TypeScript, ESLint, Prettier, Pint, Vite, route, migration, full-application PHPStan with zero current findings, Composer audit, and pnpm audit checks.

The deployed revision and Vite manifest are verified against GitHub `main` and the release package during every Hostinger activation. The live site responds successfully at `/up`; the authenticated non-destructive cycle passes English/Arabic administration, every custom operational record, private PDF downloads, contracts, statements, editable DOCX files, valid XLSX workbooks, maintenance closeouts, security headers, and the core resource cycle. Dashboard hierarchy now matches the shell breakpoint: 390px, 768px, and 1024px drawer-width layouts put current work before KPI summaries, while 1440px retains the dense KPI-first overview. Live Arabic 1024px verification moved the work workspace from 763px to 281px, placed metrics at 724px, and kept zero horizontal overflow. Superadmins always receive a compact bilingual data-source label beside property scope; production explicitly identifies the 12 showcase portfolios included in KPI totals, while owner and manager payloads remain free of global platform context. The release rebuilt Composer's complete `--no-dev --classmap-authoritative` autoloader under PHP 8.4, confirmed no pending migrations, rebuilt Laravel caches, synchronized operational statuses, reset OPcache, and removed the one-time activator. SMTP, the one-minute Hostinger scheduler, a reconciled real opening-data import, approved legal wording, and the four-role pilot remain launch blockers.
...
- `resources/css/styles/*`: modular visual layers; 31,155 source CSS lines across route and component styles.
...
- Month, quarter, and year reporting periods update both the financial query bounds and the visible EN/AR KPI wording.
...
| Scheduled due, paid, collection rate, arrears           | Scoped `lease_installments`; exact overdue installment and affected-lease counts explain arrears without mixing in maintenance data |
...
- True open-maintenance count, completed-work confirmation queue, and create-request action.
- Compact mobile measurements of 1,503px in English and 1,469px in Arabic at 390px, with zero horizontal overflow and four bounded direct-action cards.
...
**Detail:** direct Overview, Financial, Evidence, and History tabs; source link, expense status, amount, category, vendor, date, description, property/lease/maintenance context, workflow, private PDF evidence, and audit history. Only the selected panel is mounted.
**Actions:** create, edit, void/archive, open source record, upload authorized PDF evidence, and download existing evidence.

**Create/edit workspace:** dedicated three-section financial form with explicit superadmin portfolio scope, searchable property and maintenance-request choices, pending-first creation, a live amount/posting-impact rail, required-field progress, work-order context, PDF-evidence handoff, and sticky mobile Save/Cancel actions. Edit keeps the immutable portfolio visible in context.

**Fields:** portfolio; optional asset; optional maintenance request; required category; required title; description; incurred date not after today; amount 0.01..999,999,999,999.99; required uppercase currency; vendor name; posted/pending status. New records default to pending so unreviewed costs do not immediately reduce reported net revenue.
...
**Vendor detail:** direct Overview, Workload, Schedule, Financial, and History tabs; contact and service identity, open/overdue/completed counts, assigned-property schedule pressure, bounded current and completed jobs, quoted/final cost position, audit history, and one prioritized workflow. Only the selected panel renders; manager aggregates and job cards remain inside assigned properties.
...
**Detail:** direct Overview, Access, Validity, and History tabs; protected PDF identity, linked ownership, portal eligibility, per-request authorization, issue/expiry window, immutable replacement rule, and bounded audit history. Only the selected panel renders.

**Actions:** create, edit metadata/visibility, download, upload a prefilled replacement as a new auditable PDF, and delete when authorized. Payment proofs remain inside their payment review/replacement workflow.

**Fields/rules:** attach to property/lease/payment/expense and existing record ID; type; required English/Arabic title; issue date; expiry date not before issue; tenant-portal boolean; PDF file. File must have `.pdf`, PDF MIME, a valid PDF signature, and be at most 10MB.
...
4. **CSS remains large.** It is modular, not one giant file anymore, but 31,155 source lines and a 321.19KB main bundle still require the existing 325KB style-budget gate and continued dead-style review. The dashboard remains isolated in its own 35.82KB route stylesheet.
...
| Check                            | Result                                                                                                                                                    |
| -------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Fresh PHPUnit run                | 671 passed, 42,128 assertions                                                                                                                             |
...
| `/leases/create` · `leases.create`                                       | Create lease and schedule.       | Explicit portfolio, searchable tenant/rental selection, draft-first status, frequency/dates, blank-until-entered rent, deposit/tax/discount/currency/billing day, live summary/checklist, and collapsed optional terms/notes. Superadmins start without an implicit portfolio or contract party. | Save/cancel; schedule created transactionally only after the required workflow is complete.                  | SA/O/M scoped.                                            |
...
| `/leases/{lease}/edit` · `leases.edit`                                   | Controlled contract update.      | Dedicated focused controls for allowed status transition, signed date, notice, notes, and bilingual terms, with live context and validation.                                                                                                                                                     | Update/cancel.                                                                                               | SA/O/M scoped.                                            |
| `/leases/{lease}/renew` · `leases.renew`                                 | Create linked renewal draft.     | Dedicated renewal form with the source parties/rental prefilled, guarded next-term dates, financial contract fields, live summary, and required-step checklist.                                                                                                                                  | Save one linked replacement draft.                                                                           | SA/O/M scoped; one replacement per source.                |
...
| `/payments` · `payments.index`                                           | Payment register.                | Reference/type/method/status, tenant/lease/property, received date, amount/allocation, payment-proof state and file count, filters, pending-proof KPI, XLSX.                                                                                                                                     | Open, create, edit pending data, review pending proof, receipt, void.                                        | SA/O/M scoped; proof queue respects assigned properties.  |
| `/payments/create` · `payments.create`                                   | Post or stage manual payment.    | Explicit searchable lease selection, type, method, status, reference, date, positive amount, notes; identity/currency derive from lease. Superadmins start with no implicit portfolio or contract.                                                                                               | Save/cancel after the guided checklist is genuinely complete.                                                | SA/O/M scoped.                                            |
...
| `/expenses/create` · `expenses.create`                               | Record operating cost.                                | Dedicated financial form with explicit portfolio, searchable property/request source, category, title/description, date, amount/currency, vendor, pending-first status, live posting impact, checklist, and PDF-evidence next step.                                                    | Save/cancel through desktop header or sticky mobile actions.                                                                 | SA/O/M scoped.                                                                                                                                    |
...
| `/expenses/{expense}/edit` · `expenses.edit`                         | Correct a mutable cost record.                        | Same dedicated financial form with immutable portfolio context, current accounting impact, searchable source links, required progress, and evidence handoff.                                                                                                                           | Update/cancel; void records remain locked.                                                                                   | SA/O/M scoped; assigned-property boundary enforced.                                                                                               |
...
| Management dashboard | Desktop keeps property scope, six operational KPIs, then one URL-backed workspace at a time. Through 1199px the order becomes scope, work, setup, then KPIs so a real action appears before the summary cards across the complete drawer-width range. Today, Portfolio performance, and superadmin-only System controls stay directly visible; each mounts one inner panel at a time. Portfolio shows the two highest-risk properties before handing the full comparison to Portfolio Control. System separates readiness, activity, company footprint, and website control with URL persistence. All dashboard tab layers follow LTR/RTL arrow direction plus Home/End. Outstanding rent reports exact overdue installments and affected leases instead of unrelated service volume, and financial labels follow the selected month, quarter, or year in EN/AR. |
...
| Expenses             | Dedicated pending-first create/edit form with searchable scope, live posting impact, checklist, and PDF-evidence handoff; dedicated cost decision record with direct Overview/Financial/Evidence/History tabs, one workflow action, linked operational context, and private PDF evidence.                                                                                                                                         |
