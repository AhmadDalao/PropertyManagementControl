...
- Playwright and axe cover 70 scenarios at 390, 768, 1024, and 1440 pixels. Primary routes, Opening Data, Launch Readiness report controls, every Property Operating Report tab, and the tenant Account Statement have no page-level horizontal overflow. Browser titles use the localized product name instead of framework starter branding, the operations dashboard mounts one keyboard-accessible URL-backed workspace and operational queue at a time, and the tenant command center keeps its payment, maintenance, and contract panels fully translated in Arabic.
