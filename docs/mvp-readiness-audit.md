...
- Revision `27f3db2c04b54b41d7eded958e2d43ef355e050f` passes 657 PHP tests with 38,916 assertions, zero PHPStan errors against the accepted baseline, and all 70 Playwright/axe scenarios.
...
- Owners and managers run renewals from one property-scoped expiry schedule with contact dates, days remaining, renewal state, replacement-contract links, outstanding balances, and direct lease actions. The same current-snapshot scope downloads as PDF, DOCX, or XLSX; unlike currencies remain separate and foreign portfolios never enter the files.
...
- Owners and managers start each day in one prioritized Action Center rather than checking five separate modules. Collections, maintenance, renewals, move-outs, and expiring documents retain their original scoped source records and direct action pages; the combined queue adds property, responsible-person, priority, type, six-record default pagination, and a single Daily Operations Brief menu without duplicating workflow state. Compact decision cards preserve due state, status, financial value, assignment, and direct actions while cutting the seeded 390px page height by 51%. Its PDF, editable DOCX, and XLSX downloads use the exact visible filters and authorization scope, disclose bounded PDF detail, keep unlike currencies separate, and exclude foreign portfolios.
...
- Playwright and axe cover 71 scenarios at 390, 768, 1024, and 1440 pixels. Primary routes, Opening Data, Launch Readiness report controls, every Property Operating Report tab, and the tenant Account Statement have no page-level horizontal overflow. Browser titles use the localized product name instead of framework starter branding, the operations dashboard mounts one keyboard-accessible URL-backed workspace and operational queue at a time, all dashboard tab layers follow LTR/RTL arrow direction, and the tenant command center keeps its payment, maintenance, and contract panels fully translated in Arabic.
...
- The main CSS bundle is 320.21 KB before gzip, below the 325 KB release ceiling. Opening Data, Property Operating Reports, the 10.72KB Action Center stylesheet, Map, and other heavy route styles/scripts remain lazy chunks.
...
- PDF records can carry issue and expiry dates. Expired and 30/90-day renewal risks remain visible in the bilingual document directory, detail page, XLSX register, report library, and scoped Action Center instead of relying on staff memory.
