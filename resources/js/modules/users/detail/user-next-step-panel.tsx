...
export function UserNextStepPanel({
    workflow,
}: Pick<UserDetailPage, 'workflow'>) {
    const { t, text } = useTranslator();
    const actions = workflow.actions ?? [];

    return (
        <section
            className={`pmc-user-next-step is-${workflow.tone ?? 'muted'}`}
            aria-labelledby="user-next-step-title"
        >
            <header>
                <span aria-hidden="true">
                    <i className={`bi ${workflow.icon ?? 'bi-person-check'}`} />
                </span>
                <div>
                    <small>{text(workflow.eyebrow)}</small>
                    <h2 id="user-next-step-title">{text(workflow.title)}</h2>
                </div>
